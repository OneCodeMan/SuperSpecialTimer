...
final class TimerViewModel: ObservableObject {
    ...
    var displayTime: String {
        let remainingTime = durationSeconds - elapsedSeconds
        return remainingTime.formatted(.time(pattern: .minuteSecond(padMinuteToLength: 2)))
    }
    
    var progress: Double {
        let elapsed: Duration = .milliseconds(elapsedMilliseconds)
        return min((durationSeconds - elapsed) / durationSeconds, 1.0)
    }
